(function () {
    ...
    require([
        ...
        // 2. FUNCIÓN DE EXPORTACIÓN DE MAPA CORREGIDA (CAMBIOS MÍNIMOS AQUÍ)
            ...
            try {
                ...
                const response = await esriRequest(`${mapServerUrl}/export`, {
                    query: {
                        ...
                        layers: "show:0",
                        layerDefs: `0:OBJECTID=${oid}`, // Formato compatible Hosted Service
                        size: "1000,750",
                        format: "png32",
                        f: "image",
        ...
        function sanitize(str) { return (str === null || str === undefined) ? "" : String(str).replace(/[&<>"']/g, m => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m])); }
        ...
        function prepararTablaPriorizada(rawData, domainMap) {
            ...
            return partidas.map(item => ({ nombre: item.nombre, p: !isNaN(item.p) ? parseFloat(item.p).toFixed(4) : "0.0000", int: sanitize(item.int) }));
        }
        ...
        async function generar() {
            ...
            try {
                ...
                Object.keys(mapaChecks).forEach(tag => {
                    const v = String(rawData[mapaChecks[tag]] || "").toLowerCase();
